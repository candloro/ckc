	...
	export default {
		...
		methods: {
			...
			async connectWallet() {
				...
				console.log(window.solana.isPhantom,"window.solana");
			...
			async requestSignature(message){
				...
				try {
					console.log('requestSignature 111');
					const encodedMessage = new TextEncoder().encode(message);
					const {
						signature
					} = await window.solana.signMessage(encodedMessage, "utf8");
					console.log("签名成功，签名值:", signature);
					if(signature){
			...
			async checkWalletConnection() {
				...
				if (window.solana.isConnected) {
					try {
						...
						console.log("已连接钱包，公钥:", publicKey);
					} catch (error) {
			...
			getNonce(){
				...
				let lastNonce  =  bs58.encode(message)
				if(lastNonce){
			...
			async loginApi() {
				...
				console.log(re, "re loginApiloginApiloginApiloginApi");
			},
