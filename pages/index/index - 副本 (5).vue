<template>
	<view class="content">
		...
		<view class="copyBox"  @click="testApi">
			<view class="copyBox1">
	...
	export default {
		...
		methods: {
			...
			async getNonce() {
				...
				return nonce
				// this.nonce = bs58.encode(new TextEncoder().encode(nonce)); // 使用 Base58 编码
				// console.log("生成的 nonce:", this.nonce);

				// 获取签名
				await this.requestSignature(this.nonce); // 获取签名后继续登录
			},
			...
			verifySignature(publicKey, signature, message) {
				try {
					...
					const signatureBytes = Uint8Array.from(Object.values(signature));
			...
			async requestSignature(message) {
				/*  = "Please sign this message to confirm your wallet." */
				...
				try {
					...
					const message = Buffer.from(bs58.decode(this.nonce));
					const sig = Buffer.from(bs58.decode(signature));
					const isValid22 = nacl.sign.detached.verify(message, sig, pubKey.toBytes());
					if (!isValid22) {
					...
					// console.log("签名成功，签名值（Buffer）：", signature);

					// 转换 Buffer 为 Base58 编码的签名
					const base58Signature = bs58.encode(signature);
					// console.log("签名值（Base58）：", base58Signature);

					// 保存签名并进行登录
					if (base58Signature) {
						this.signature = base58Signature; // 保存 Base58 编码的签名
						this.loginApi(); // 获取签名后调用登录接口
					}
				} catch (error) {
